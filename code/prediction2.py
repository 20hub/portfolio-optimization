...
newpreds = [CBIpre, NETEXPpre, UNRATEpre]
newpreds = pd.concat(newpreds, axis=1)
newpreds.columns = ['CBI', 'NETEXP', 'UNRATE']

newvarb = [CBI, NETEXP, UNRATE, GDP]
newDATA = pd.concat(newvarb, axis=1)
newDATA.columns = ['CBI', 'NETEXP', 'UNRATE', 'GDP']

# Choose the model without PPI and CPI
formu3 = 'GDP ~ CBI + NETEXP + UNRATE'
mod_pre3 = ols(formu3, data=DATA).fit()
print(mod_pre3.summary())

new_anova = anova_lm(mod_pre3)
print("New ANOVA is:")
print(new_anova)

new1 = list(newpreds.iloc[0])
new1.insert(0, 1)
GDP1 = mat(mod_pre3.params) * mat(new1).T
new1.remove(1)
...
formu3_2 = 'GDP ~ CBI + NETEXP + UNRATE'
mod_pre3_2 = ols(formu3_2, data=DATA).fit()

new2 = list(newpreds.iloc[1])
new2.insert(0, 1)
GDP2 = mat(mod_pre3_2.params) * mat(new2).T
new2.remove(1)
...
formu3_3 = 'GDP ~ CBI + NETEXP + UNRATE'
mod_pre3_3 = ols(formu3_3, data=DATA).fit()

new3 = list(newpreds.iloc[2])
new3.insert(0, 1)
GDP3 = mat(mod_pre3_3.params) * mat(new3).T
new3.remove(1)
